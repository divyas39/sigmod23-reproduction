...
def fetch_IonQ_rebase_pass():
    ionq_token = config.configuration["ionq-token"]
    set_ionq_config(ionq_token) # Set IonQ API token
...
def fetch_IBMQ_rebase_pass():
    ibmq_token = config.configuration["ibmq-token"]
    set_ibmq_config(ibmq_api_token=ibmq_token) # Set IBMQ API token
